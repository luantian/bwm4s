...
const selectStatus = (index) => {
  ...
  if (index === 0) {
    ...
    frontCar.position.set(0, 0, 0);
    hoodCar.position.set(0, 0, 0);
    glassCar.position.set(0, 0, 0);
  } else {
    carBody.position.set(3, 0, 0);
    frontCar.position.set(3, 0, 0);
    hoodCar.position.set(3, 0, 0);
    glassCar.position.set(3, 0, 0);
  }
};
onMounted(() => {
  // 把渲染器插入到dom中
  ...
  loader.load("./model/bmw01.glb", (gltf) => {
    ...
    bmw.traverse((child) => {
      ...
      // 判断是否是宝马双肾
      if (child.isMesh && child.name.includes("宝马双肾")) {
