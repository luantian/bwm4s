...
const selectStatus = (index) => {
  ...
  if (index === 0) {
    ...
    tl.to(
      [frontCar.position, hoodCar.position, glassCar.position, Torus.position],
      {
        x: 0,
        duration: 0.4,
      }
    );
    tl2.to([carBody.position], {
      y: 0,
      duration: 0.4,
    });
  } else {
    ...
    const tl2 = gsap.timeline();
    tl.to(
      [frontCar.position, hoodCar.position, glassCar.position, Torus.position, ...rearTailLights.map((item) => item.position)],
      {
        x: 2,
        duration: 0.4,
      }
...
onMounted(() => {
  ...
  loader.load("./model/bmw01.glb", (gltf) => {
    ...
    bmw.traverse((child) => {
      ...
      // 三合一后尾灯
      if (child.isMesh && (child.name === 'Box01001' || child.name === 'Mesh' || child.name === 'Mesh_1')) {
