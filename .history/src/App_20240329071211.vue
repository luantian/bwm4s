...
const selectStatus = (index) => {
  if (index === 0) {
    ...
    tl.to([frontCar.position, hoodCar.position], {
      y: 0,
      duration: 0.4,
    }).to([glassCar.position, ...rearTailLights.map((item) => item.position)], {
      y: 0,
      duration: 0.4,
      ...
      x: 0,
      duration: 0.4,
    }).to([licensePlate.position], {
      z: 0,
      duration: 0.4,
    })
    gsap.to([carBody.position], {
      z: 0,
      duration: 0.4
    })
  } else {
...
onMounted(() => {
  ...
  loader.load("./model/bmw01.glb", (gltf) => {
    ...
    bmw.traverse((child) => {
      ...
      // 判断是否是车身
      if (child.isMesh && child.name === 'Mesh002') {
        carBody = child;
      ...
      // ChamferBox04 车牌照
      if (child.isMesh && child.name === "ChamferBox01") {
        licensePlate = child;
      }
