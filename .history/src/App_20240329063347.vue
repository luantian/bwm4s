...
let wheels = [];
let bmw, carBody, frontCar, hoodCar, glassCar;
// 创建材质
...
const selectStatus = (index) => {
  ...
  if (index === 0) {
    ...
    tl.to([frontCar.position, hoodCar.position], {
      x: 0,
      duration: 0.4,
    }).to([carBody.position, glassCar.position, ...rearTailLights.map((item) => item.position)], {
      y: 0,
      duration: 0.4,
    }).to([...Torus.map((item) => item.position)], {
      z: 0,
      duration: 0.4,
    });;
    gsap.to([chassis.position], {
      x: 0,
      duration: 0.4,
    })
  } else {
    ...
    tl2.to([carBody.position, glassCar.position, ...rearTailLights.map((item) => item.position)], {
      y: 2,
      duration: 0.5,
    });
    tl3.to([...Torus.map((item) => item.position)], {
      z: 2,
      duration: 0.6,
    });
...
onMounted(() => {
  ...
  loader.load("./model/bmw01.glb", (gltf) => {
    ...
    bmw.traverse((child) => {
      ...
      // 两个车前大灯
      if (child.isMesh && child.name.includes("Torus")) {
        Torus.push(child);
      }
      ...
      if (
        ...
        rearTailLights.push(child);
      }

      // Plane08 底盘
      if (child.isMesh && child.name === "Plane08") {
        chassis = child;
      }

      // ChamferBox04 车牌照
      if (child.isMesh && child.name === "ChamferBox04") {
        licensePlate = child;
      }
